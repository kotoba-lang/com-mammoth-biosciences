(ns mammoth_biosciences.main
  "Kotodama WASM entrypoint for the Mammoth_biosciences clean-room actor (L4) — Clojure port.
  ...
  facts (`mammoth_biosciences.<Entity>/<field>`); `*store*` is the in-memory
  materialization used by the contract test and by the WASM runtime before a
  live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mammoth_biosciences")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"mammoth_biosciences.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mammoth_biosciences-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mammoth_biosciences-compat/ready)
