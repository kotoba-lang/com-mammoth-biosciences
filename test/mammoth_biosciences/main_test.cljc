(ns mammoth_biosciences.main-test
  "Contract + behavioral test for the mammoth_biosciences-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "mammoth_biosciences-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'mammoth_biosciences.main-test)]
